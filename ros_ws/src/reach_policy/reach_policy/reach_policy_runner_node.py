...
from geometry_msgs.msg import Pose, PoseStamped # Used for EE position and Goal position (Note: PointStamped is replaced by PoseStamped)
import torch
...
class ReachPolicyRunnerNode(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            if 'model_state_dict' in checkpoint:
                ...
                self.get_logger().info("Found full Actor-Critic weights under 'model_state_dict'. Filtering Actor keys...")
    ...
    def _target_pose_callback(self, msg: PoseStamped):
        ...
        self.target_ori = np.array([pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w])
        self._calculate_errors()
    ...
    def _get_observation(self) -> torch.Tensor:
        ...
        if len(obs_np) != OBS_DIM:
             ...
             self.get_logger().error(f"FATAL: Observation dimension mismatch: {len(obs_np)} != {OBS_DIM}. Check filtering logic or policy configuration.")
